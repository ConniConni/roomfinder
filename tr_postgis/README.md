...
### 4.2.4 validate_target_point (ID: SV-XXX)

| ID    | 分類 | テスト項目（観点）                           | 期待される結果                                                                                                                                  |
| :---- | :--- | :------------------------------------------- | :---------------------------------------------------------------------------------------------------------------------------------------------- |
| SV-N1 | 正常 | 基準点が正しく取得できること                 | ・WKT形式（POINT(経度 緯度)）で文字列が返ること                                                                                                 |
| SV-N2 | 正常 | 境界値の点も基準点が正しく取得できること     | ・境界値（20.0や154.0など）を含んだWKT形式の文字列が返ること                                                                                    |
| SV-E1 | 異常 | 緯度が範囲外の場合、SystemExitが発生すること | ・エラーログで"【設定値エラー】基準点は北緯20°〜45°、東経122°〜154°の間の数値で設定してください。"と表示されること<br>・終了コードが1となること |
| SV-E2 | 異常 | 経度が範囲外の場合、SystemExitが発生すること | ・エラーログで"【設定値エラー】基準点は北緯20°〜45°、東経122°〜154°の間の数値で設定してください。"と表示されること<br>・終了コードが1となること |

### 4.2.5 validate_execution_settings (ID: SVE-XXX)

| ID     | 分類 | テスト項目（観点）                                 | 期待される結果                                      |
| :----- | :--- | :------------------------------------------------- | :-------------------------------------------------- |
| SVE-N1 | 正常 | 正しい数値（int/float）が設定されている            | 値がそのまま返却されること                          |
| SVE-N2 | 正常 | 数字の文字列（"100"等）が設定されている            | 適切な数値型（int/float）に変換されて返却されること |
| SVE-E1 | 異常 | 負の数値が設定されている                           | エラーログ出力後、ステータス 1 で終了すること       |
| SVE-E2 | 異常 | 0 （文字列含む）が設定されている                   | エラーログ出力後、ステータス 1 で終了すること       |
| SVE-E3 | 異常 | 数値に変換できない文字列（"st"等）が設定されている | エラーログ出力後、ステータス 1 で終了すること       |
| SVE-E4 | 異常 | 変換不可能な型（list等）が設定されている           | エラーログ出力後、ステータス 1 で終了すること       |

### 4.2.6 main (ID: SM-XXX)
...
| SM-N3 | 正常 | 正クエリの境界値をテスト                                       | テスト用のDBでクエリを実行すると3件、id=1,2,3のものが取得できること                                                            |
| SM-N4 | 正常 | 正クエリの境界値をテスト(設定値変更)                           | テスト用のDBでクエリを実行すると1件、id=1のものが取得できること                                                                |
