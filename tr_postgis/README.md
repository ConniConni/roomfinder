...
| SM-E1 | 異常 | `OperationalError` 発生時、適切にログ出力して終了すること      | ・ERRORログに「【DB接続エラー】設定を見直してください。...」が出力されること<br>・プログラムが異常終了（クラッシュ）しないこと |
| SM-E2 | 異常 | `ProgrammingError` 発生時、適切にログ出力して終了すること      | ・ERRORログに「【SQL実行エラー】クエリの内容を確認してください。...」が出力されること<br>・「DB切断」ログが出力されること      |
| SM-E3 | 異常 | その他 `psycopg2.Error` 発生時、適切にログ出力して終了すること | ・ERRORログに「【その他DBエラー】:...」が出力されること<br>・「DB切断」ログが出力されること                                    |
| SM-C1 | 共通 | 成功・失敗に関わらず、最終的に `conn.close()` が呼ばれること   | ・正常終了、各種DBエラー発生時のいずれの場合も「DB切断」ログが出力されること（リソースが解放されること）                       |
